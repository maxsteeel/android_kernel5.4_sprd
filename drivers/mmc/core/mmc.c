...
#include "sd_ops.h"
#include "pwrseq.h"
#include "../host/sdhci-sprd-powp.h"
...
static struct memory_type_list sprocomm_memtype[] = {
	{"ISOCOM","ISOCOM MEMA064G M0D02"},
	{"SCA64G","SCY E64GDSAA2ABE00"},
	{"A3A551","FORESEE FEMDNN032G-A3A55"},
	{"ARJ11X","Biwin BWCTARJ11X32G"},
	{"SDS6S2","HIKSEMI HSEMSDS6S2B32G"},
};
static ssize_t mmc_memorytype_show (struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	uint32_t count;
	struct mmc_card *card = mmc_dev_to_card(dev);
    count = sizeof(sprocomm_memtype)/sizeof(struct memory_type_list);
