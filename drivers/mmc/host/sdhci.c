...
void mmc_debug_print(struct mmc_debug_info *info)
{
	u32 read_speed = 0;
	u32 write_speed = 0;
	...
	if ((ktime_to_ms(ktime_get()) - info->cnt_time) > (10000ULL)) {
		...
		pr_info("|__speed%7s: read_speed = %d.%d MB/s, write_speed = %d.%d MB/s\n",
			info->name, read_speed / 100, read_speed % 100,
			write_speed / 100, write_speed % 100);
...
void mmc_debug_update(struct sdhci_host *host, struct mmc_command *cmd, u32 type)
{
	...
	if (!type) {
		...
		if (index >= 11)
			pr_info("%s: cmd respond over 1s! cmd= %d blocks= %d arg= %d resp= %d\n",
				info->name, info->cmd, info->blocks,
				info->arg, sdhci_readl(host, SDHCI_RESPONSE));
		...
		if (index >= 11)
			pr_info("%s: data respond over 1s! cmd= %d blocks= %d arg= %x\n",
				info->name, info->cmd, info->blocks, info->arg);

		mmc_debug_calc(info);
		info->start_time = 0;
		mmc_debug_print(info);
	}
}
...
#ifdef CONFIG_SPRD_DEBUG
		mmc_debug_update(host, 0, intmask);
#endif
