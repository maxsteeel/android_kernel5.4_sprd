...
static int tlsx6x_comb_get_running_cfg(u16 *ptcfg, u16 addr)
{
	...
	while (++retry < 5) {
       ...
       tlsc_info("retry = %d:   err_type = %d /n",retry, err_type);
	return err_type;
...
int tlsc6x_do_update_ifneed(void)
{
	...
	if(3 == tlsc_vendor_id && 97 == tlsc_project_id) {
            fupd = fw_file_tlsc6x_3_97;
            fw_size = ARRAY_SIZE(fw_file_tlsc6x_3_97);
            ret = tlsc6x_update_compat_ctl((u8 *) fupd, fw_size);
        }
        else if(24 == tlsc_vendor_id && 139 == tlsc_project_id) {
            fupd = fw_file_tlsc6x_24_139;
            fw_size = ARRAY_SIZE(fw_file_tlsc6x_24_139);
            ret = tlsc6x_update_compat_ctl((u8 *) fupd, fw_size);
        }
        else if(24 == tlsc_vendor_id && 153 == tlsc_project_id) {
            fupd = fw_file_tlsc6x_24_153;
            fw_size = ARRAY_SIZE(fw_file_tlsc6x_24_153);
            ret = tlsc6x_update_compat_ctl((u8 *) fupd, fw_size);
        }
    #else
        fupd = fw_file_tlsc6x_3_72;
        fw_size = ARRAY_SIZE(fw_file_tlsc6x_3_72);
	ret = tlsc6x_update_compat_ctl((u8 *) fupd, fw_size);
    #endif
...
int tlsc6x_tp_dect(struct i2c_client *client)
{
	...
	if (g_mccode == 0xff) {
		...
		RSP_ADDR = 0x809f40;
		g_tlsc6x_boot_ver = 0;
        tlsc_info("g_mccode == 0xff  return 0 \n"); 
		return 0;
	}
	...
	if (tlsc6x_get_running_cfg(tl_buf_tmpcfg) == 0) {
		...
		tlsc_vendor_id = (((g_tlsc6x_cfg_ver>>9)&0x7F) | ((g_tlsc6x_cfg_ver>>22&0x03)<<7));
		tlsc_project_id = ((g_tlsc6x_cfg_ver&0x01FF)| ((g_tlsc6x_cfg_ver>>20&0x03)<<9));
		tlsc_info("g_tlsc6x_chip_code is 0x%x tlsc_cfg_version is 0x%x tlsc_vendor_id is 0x%x tlsc_project_id is 0x%x\n",g_tlsc6x_chip_code,tlsc_cfg_version,tlsc_vendor_id,tlsc_project_id);
	} else {
		//tlsc_info("txtx get fail\n");
	...
	ret = tlsc6x_do_update_ifneed();
       ...
       tlsc_info("ret = tlsc6x_do_update_ifneed00  ret = 0x%x\n",ret);
	 if(0 != ret) {
            tlsc_info("ret = tlsc6x_do_update_ifneed  ret = 0x%x\n",ret);
			//return 0;
        }
#endif
	ret = tlsc6x_set_nor_mode();
