...
#define REPORT_VIRTUAL_KEY_WITH_VALUE
#define TLSC_TPD_PROXIMITY_SHAUNGDAN
...
#endif
#ifdef REPORT_VIRTUAL_KEY_WITH_VALUE
...
struct key_data tlsc6x_key_data[MAX_KEY_NUM] = {
	{KEY_BACK, 340, 460, 855, 945},
	{KEY_HOMEPAGE, 180, 300, 855, 945},
	{KEY_MENU, 20, 140, 855, 945},
};
#endif

#ifdef REPORT_VIRTUAL_KEY_WITH_VALUE
static int find_key_id(u16 x, u16 y)
{
    u16 i = 0;
    int key_id = 0;

    for(i = 0; i < MAX_KEY_NUM; i++){
        if((tlsc6x_key_data[i].x_min < x) && (x < tlsc6x_key_data[i].x_max)&&(tlsc6x_key_data[i].y_min < y) && (y < tlsc6x_key_data[i].y_max)){
            key_id = i+1;
            break;
        }
    }
...
#else
static ssize_t show_tlsc_facemode(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	char *ptr = buf;
#ifdef TLSC_TPD_PROXIMITY
...
static ssize_t store_tlsc_facemode(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
#ifdef TLSC_TPD_PROXIMITY
	if (buf[0] == '0') {
		printk("store_tlsc_facemode shuangdan 0\n");
		tlsc6x_prox_ctrl(0);
		//ps off

	} else if (buf[0] == '1') {
		printk("store_tlsc_facemode shuangdan 1\n");
		//ps on
		tlsc6x_prox_ctrl(1);
	}
#endif
...
static struct kobj_attribute tp_face_mode_attr = {
    ...
    .show = &show_tlsc_facemode,
    .store = &store_tlsc_facemode,
};
...
#ifdef TLSC_TPD_PROXIMITY_SHAUNGDAN
static ssize_t show_tlsc_facemode(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	char *ptr = buf;
#ifdef TLSC_TPD_PROXIMITY
...
static void tlsc6x_virtual_keys_init2(void)
{
	int ret = 0;
	struct kobject *properties_kobj;
...
{
	TLSC_FUNC_ENTER();
	printk("shuangdan tlsc6x_prox_ctrl enable =%d\n",enable);
...
static int tlsc6x_update_data(void)
{
	...
	//tlsc_info("tpd-proximity code:%x  tpd_prox_active = %d \n",buf[1], tpd_prox_active);
#ifdef TLSC_TPD_PROXIMITY
...
#ifdef REPORT_VIRTUAL_KEY_WITH_VALUE
        if((y > pdata->y_res_max) && (1==event->touch_point)){
            int key_id = find_key_id(x, y);

            if(key_id){
                key_state_flag = key_id;
                input_report_key(data->input_dev, BTN_TOUCH, 1);
                input_report_key(data->input_dev, tlsc6x_key_data[key_state_flag-1].key, 1);
            }
            continue;

        }else if(key_state_flag){
            input_report_key(data->input_dev, tlsc6x_key_data[key_state_flag-1].key, 0); 
            key_state_flag = 0;
        }
#endif
...
static int tlsc6x_do_resume(void)
{
	...
	hrtimer_start(&tpd_esd_kthread_timer_status, ktime_set(30, 0), HRTIMER_MODE_REL);
#endif
...
static int esd_check_work(void)
{
	...
	if (ret < 0) {
		tlsc_err("esd check fail, force reset!\n");
		tlsc6x_tpd_reset_force();
		tlsc6x_clear_report_data(g_tp_drvdata);
...
static ssize_t tlsc_tp_info(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	switch(tlsc_vendor_id){
		case 24:{
			ptr += sprintf(page, "YIXING:CHSC6540:YY_%03d_%03x\n ", tlsc_cfg_version, g_tlsc6x_boot_ver);
		}break;
		case 3: {
			ptr += sprintf(page, "YUYE:CHSC6540_TBD:YY_%03d_%03x\n ", tlsc_cfg_version, g_tlsc6x_boot_ver);
		}break;
		default: {
...
static int tlsc6x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_tp_drvdata->needKeepRamCode = g_needKeepRamCode;
       ...
       tlsc_info("g_is_telink_comp is 0x%x\n",g_is_telink_comp); 
	if (g_is_telink_comp) {
		tlsc6x_tpd_reset();
	} else {
...
//	input_set_abs_params(input_dev, ABS_MT_PRESSURE, 0, 127, 0, 0);
	...
	ps_wakelock = wakeup_source_create("ps_wake_lock");
	wakeup_source_add(ps_wakelock);
#endif

#ifndef REPORT_VIRTUAL_KEY_WITH_VALUE
	if (pdata->have_virtualkey){
		printk("entry:tlsc6x_virtual_keys_init\n");
		tlsc6x_virtual_keys_init();
	}
#endif
		tlsc6x_virtual_keys_init2();
...
/*	if (calibration_mode) {
		printk("[tlsc6x] %s: Calibration Mode! setup suspend\n",__func__);
		tlsc6x_irq_disable();
...
exit_input_register_device_failed:
	input_free_device(input_dev);
        wakeup_source_remove(ps_wakelock);
