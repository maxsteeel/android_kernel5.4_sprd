...
void tp_reset_control(int enable)
{
	if(fts_data && fts_data->pdata){
    	FTS_DEBUG("tp reset %d", enable);
	    gpio_direction_output(fts_data->pdata->reset_gpio, enable);
	}
}
EXPORT_SYMBOL_GPL(tp_reset_control);
		    ...
		    struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((buf[0] == '1') && !fts_data->suspended)
		fts_ts_suspend(fts_data->dev);
	else if ((buf[0] == '0') && fts_data->suspended)
...
static int tp_sysfs_init(struct fts_ts_data *ts_data)
{
	int ret = 0;
	tp_ctrl_kobj = kobject_create_and_add("touchscreen", NULL);
	if (!tp_ctrl_kobj){
		FTS_ERROR("Create /sys/touchscreen failed!");
        return -ENOMEM;
	}else
		FTS_INFO("Create /sys/touchscreen succesfully!");

	ret = sysfs_create_group(tp_ctrl_kobj, &tp_attr_group);
	if(ret){
		FTS_ERROR("Create /sys/touchscreen/ts_suspend failed!");
		kobject_put(tp_ctrl_kobj);
	}else
		FTS_INFO("Create /sys/touchscreen/ts_suspend succesfully!");
...
static ssize_t sprocomm_read_tp_info(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	len = read_tp_info(tp_info, sizeof(tp_info));

	ret = copy_to_user(buffer,(char *)tp_info,len);
	if(ret)
		return ret;
...
static int sprocomm_create_proc_tp_info(void)
{
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	ret = sprocomm_create_proc_tp_info();
	if(ret){
		FTS_ERROR("create procfs node fail");
	}

	ret = tp_sysfs_init(ts_data);
	if(ret){
		FTS_ERROR("create suspend sysfs node faile");
	}
...
static void cali_fts_ts_suspend(struct work_struct *work)
{
	if(!fts_data)
		return;
...
static int fts_ts_probe(struct spi_device *spi)
{
...
#if 0
	if (calibration_mode) {
		FTS_INFO("Calibration Mode! Don't register touchscreen driver\n");
		return -ENODEV;
	}
#endif
	...
	if(calibration_mode){
		INIT_DELAYED_WORK(&disable_work, cali_fts_ts_suspend);
		schedule_delayed_work(&disable_work, msecs_to_jiffies(5000));
	}
...
static int __init fts_ts_init(void)
{
    ...
    {
    		printk("%s, !!!charger boot mode no need to loader tp driver,direct return\n",__func__);
		return 0;
    }
