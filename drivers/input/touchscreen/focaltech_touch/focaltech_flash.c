...
struct upgrade_module module_list[] = {
    {FTS_MODULE_ID, FTS_VENDOR_NAME, FTS_MODULE_NAME, fw_file, sizeof(fw_file), 0},
    {FTS_MODULE2_ID, FTS_VENDOR2_NAME, FTS_MODULE2_NAME, fw_file2, sizeof(fw_file2), 0},
    {FTS_MODULE3_ID, FTS_VENDOR3_NAME, FTS_MODULE3_NAME, fw_file3, sizeof(fw_file3), 0},
};
...
int fts_fw_resume(bool need_reset)
{
    ...
    if (FTS_FW_REQUEST_SUPPORT) {
        snprintf(fwname, FILE_NAME_LENGTH, "%s%s.bin", \
                 FTS_FW_NAME_PREX_WITH_REQUEST, upg->module_info->module_name);
...
static bool ft8006s_GUOXIAN_module = false;
static int lcd_name_check(char *str)
{
	if (str != NULL && !strncmp(str, "lcd_ft8006s_gx_mipi", strlen("lcd_ft8006s_gx_mipi")))
		ft8006s_GUOXIAN_module = true;
	else
		ft8006s_GUOXIAN_module = false;
		
	return 0;
}
__setup("lcd_name=", lcd_name_check);
...
static int fts_fwupg_get_module_info(struct fts_upgrade *upg)
{
	...
	/************************************************
	*Jiangguijuan add,2020,10,09 for distingwish different modules.GENRPRO and GUOXIAN
	*ONLY for E5531N Project, non-flash,module_id need transfer from LCD driver(gpio or lcm_id or ...);
	*/
	if(ft8006s_GUOXIAN_module)
		upg->module_id = 0xe4;
	else
		upg->module_id = 0x51;
	/*Jiangguijuan add,2020,10,09 for distingwish different modules.GENRPRO and GUOXIAN
...
static void fts_fwupg_work(struct work_struct *work)
{
    ...
    if (ret < 0) {
        ...
        ret = fts_read_reg(FTS_REG_CHIP_ID, &chip_id);
        msleep(50);
        ret = fts_read_reg(FTS_REG_FW_VER, &upg->module_info->fw_ver);
        FTS_INFO("read chip id:0x%02x", chip_id);
        FTS_INFO("read fw ver:0x%02x", upg->module_info->fw_ver);
    }
}
...
ssize_t read_tp_info(char *buf, ssize_t sz)
{
	struct fts_upgrade *upg = fwupgrade;

	return snprintf(buf, sz, "%s:%s:0x%02x\n", upg->module_info->vendor_name, upg->module_info->module_name, upg->module_info->fw_ver);
}
