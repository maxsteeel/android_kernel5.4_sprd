...
static int wcn_download_image(struct wcn_device *wcn_dev)
{
	...
	if (err < 0) {
		...
		if (wcn_check_2to1_bin(wcn_dev, firmware, &off)) {
...
static int wcn_wait_wcn_deep_sleep(struct wcn_device *wcn_dev, int force_sleep)
{
	...
	u32 wait_count_offset = (WCN_WAIT_SLEEP_MAX_COUNT - WCN_WAIT_SLEEP_DEFAULT_COUNT);
	...
	for (wait_sleep_count = 0;
		...
		if (wcn_get_sleep_status(wcn_dev, force_sleep) == 0)
			break;
		if (atomic_read(&s_wcn_device.gnss_stop_wait_sleep) == GNSS_STOP_WAIT_SLEEP &&
            !wcn_dev_is_marlin(wcn_dev) && wait_sleep_count < wait_count_offset) {
            /* Shorten the waiting time to meet the requirements of BT power on time */
            WCN_WARN("Set wait_sleep_count(%u->%d)\n",
                    wait_sleep_count, wait_count_offset);
            wait_sleep_count = wait_count_offset;
            atomic_set(&s_wcn_device.gnss_stop_wait_sleep, 0);
            continue;
        }
...
static bool wcn_is_stopping(struct wcn_device *wcn_dev)
{
    if (!wcn_dev)
        return false;

    return mutex_is_locked(&wcn_dev->power_lock);
}
...
int stop_integrate_wcn(u32 subsys)
{
    ...
    atomic_set(&s_wcn_device.gnss_stop_wait_sleep, 0);
    if (wcn_platform_chip_type() != WCN_PLATFORM_TYPE_QOGIRL6 &&
        subsys == WCN_MARLIN_BLUETOOTH && mutex_is_locked(&marlin_lock) &&
        wcn_is_stopping(wcn_get_dev_by_type(WCN_GNSS))) {
        WCN_INFO("Reduce the power down time of GNSS\n");
        atomic_set(&s_wcn_device.gnss_stop_wait_sleep, GNSS_STOP_WAIT_SLEEP);
    }
