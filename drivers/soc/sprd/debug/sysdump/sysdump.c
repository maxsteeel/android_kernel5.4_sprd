...
static int extend_section_cm4dump(void)
{
    ...
    #define CM4_DUMP_IRAM "sprd-spdump"
    struct device_node *node;
    int count = 0;
    unsigned long cm4_dump_start = 0;
    unsigned long cm4_dump_end = 0;
    const __be32 *list;
    char fname[16];
    int i;
    ...
    list = of_get_property(node, "sprd,dumpmem-info", &count);
    if (!list || !count) {
		pr_err("No sprd,dumpmem-info from node %s \n", CM4_DUMP_IRAM);
		return -2; 
    }
    
    count = count / (2 * sizeof(__be32));
    for (i = 0; i < count; i++) {
        cm4_dump_start =  be32_to_cpu(*list++);
        cm4_dump_end = cm4_dump_start + be32_to_cpu(*list++) - 1;
        sprintf(fname,"cm4dump_%d",i);
        minidump_save_extend_information(fname,cm4_dump_start,cm4_dump_end);
        pr_info("cm4_dump added to minidump section ok!!\n");
    }
    return 0;
