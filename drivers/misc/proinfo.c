...
#define SIZE_1K 1024
#define WT_PROINFO_STRING_LEN 8
#define WT_PROINFO_IMEI_LEN 8
#define WT_PROINFO_REMOTE_LOCK_VLAUE_LEN 256
#define WT_PROINFO_BLOCK_FASTBOOT_OFFSET (771 * SIZE_1K)
#define WT_PROINFO_BLOCK_FACTORY_RESET_OFFSET (WT_PROINFO_BLOCK_FASTBOOT_OFFSET + MMC_BLOCK_SIZE)
#define WT_PROINFO_REMOTE_LOCK_VALUE_OFFSET (WT_PROINFO_BLOCK_FACTORY_RESET_OFFSET + MMC_BLOCK_SIZE)
#define WT_PROINFO_1_LEN 1
#define WT_PROINFO_8_LEN 8

#define PHONE_INFO_PATH "/dev/block/platform/bootdevice/by-name/proinfo"
#define OEM_INFO_PATH "/dev/block/platform/bootdevice/by-name/miscdata"
...
//define function
WT_PROINFO_CREATE_ATTR(miscdata_clear)
...
static int wt_create_device_files(void)
{
    ...
    rc = device_create_file(proinfo_root_dev, &dev_attr_miscdata_clear);
    if (rc)
        return rc;
...
static int emmc_block_rw(int write, sector_t index, void *buffer, size_t len)
{
	...
	if (bh) {
		...
		submit_bh(REQ_OP_READ, REQ_SYNC, bh);
		wait_on_buffer(bh);
...
/*        case WT_PROINFO_color_id:
            ...
            ret = emmc_partition_rw("proinfo", 0, 0, temp_buf, SIZE_1K / 2);
            if (ret < 0) {
...
//            break;
        ...
        case WT_PROINFO_block_fastboot_mode:
            ...
            ret = emmc_partition_rw("miscdata", 0, WT_PROINFO_BLOCK_FASTBOOT_OFFSET, temp_buf, MMC_BLOCK_SIZE);
            if (ret < 0) {
                printk("%s: Read bytes from proinfo failed! %d\n", __func__, ret);
                return -1;
            }
            ...
            /*
            fp->f_pos = fp->f_pos + type * SIZE_1K;
        ...
        case WT_PROINFO_block_factory_reset:
            ...
            ret = emmc_partition_rw("miscdata", 0, WT_PROINFO_BLOCK_FACTORY_RESET_OFFSET, temp_buf, MMC_BLOCK_SIZE);
            if (ret < 0) {
                printk("%s: Read bytes from proinfo failed! %d\n", __func__, ret);
                return -1;
            }
         ...
         case WT_PROINFO_remote_lock_value:
            ...
            ret = emmc_partition_rw("miscdata", 0, WT_PROINFO_REMOTE_LOCK_VALUE_OFFSET, temp_buf, MMC_BLOCK_SIZE);
            if (ret < 0) {
                printk("%s: Read bytes from proinfo failed! %d\n", __func__, ret);
                return -1;
            }
...
/*        case WT_PROINFO_factoryreset_date:
            ...
            ret = emmc_partition_rw("miscdata",  0, type * SIZE_1K,(char *) fbuf, (unsigned long) WT_PROINFO_STRING_LEN);
            break;*/
        default:
            break;
    }
...
/*        case WT_PROINFO_sku_id:
            ...
            ret = emmc_partition_rw("proinfo", 1, 0, temp_buf, SIZE_1K / 2);
            if (ret < 0) {
       ...
       case WT_PROINFO_factoryreset_date:
           ...
           ret = emmc_partition_rw("miscdata", 1, type * SIZE_1K, (char *) buf_tmp, (unsigned long) len);
           break;
*/
       case WT_PROINFO_block_factory_reset:
           printk("%s write factory[%d] \n", __func__, type);
           ret = emmc_partition_rw("miscdata", 1, WT_PROINFO_BLOCK_FACTORY_RESET_OFFSET, (char *) buf_tmp, MMC_BLOCK_SIZE);
           break;

       case WT_PROINFO_block_fastboot_mode:
           printk("%s write factory[%d] \n", __func__, type);
           ret = emmc_partition_rw("miscdata", 1, WT_PROINFO_BLOCK_FASTBOOT_OFFSET, (char *) buf_tmp, MMC_BLOCK_SIZE);
           break;

       case WT_PROINFO_remote_lock_value:
           printk("%s write factory[%d] \n", __func__, type);
           ret = emmc_partition_rw("miscdata", 1, WT_PROINFO_REMOTE_LOCK_VALUE_OFFSET, (char *) buf_tmp, MMC_BLOCK_SIZE);
           break;

        default:
            printk("%s unknown write type[%d] \n", __func__, type);
/*
            memcpy(temp_buf, buf_tmp, len);
            ret = emmc_partition_rw("miscdata", 1, type * SIZE_1K, temp_buf, SIZE_1K / 8);
            if (ret < 0) {
