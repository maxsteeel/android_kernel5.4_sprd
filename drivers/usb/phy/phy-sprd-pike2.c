...
#if (defined(CONFIG_SPRD_PIKE2_USB2_PHY))||(defined(CONFIG_SPRD_SHARKLE_USB2_PHY))
...
static enum usb_charger_type sprd_hsphy_retry_charger_detect(struct usb_phy *x)
{
	...
	regmap_update_bits(phy->pmic,
		...
		dev_err(x->dev,"in while delay %d , phy->dp_voltage:%d\n",phy->dp,dp_voltage);
	if(dp_voltage > VOLT_LO_LIMIT) {
		do
		{
			iio_read_channel_processed(phy->dm, &dm_voltage);
			dev_err(x->dev,"in while delay %d , phy->dm_voltage:%d\n",phy->dp,dm_voltage);
...
static int sprd_hsphy_probe(struct platform_device *pdev)
{
	...
	{
		phy->dp = NULL;
		dev_warn(dev,"failed to get dp channel\n");
	}
	if (IS_ERR(phy->dm))
	{
		phy->dm = NULL;
		dev_warn(dev,"failed to get dm channel\n");
	}
#endif
